...
test('经过自己城市进入 build_decide，可建房', () => {
  ...
  p.cities.push('罗马');
  p.position = 10;
  // 掷骰落 12：rollDice 两次调用 → 需要 (1,1)：rng=0 给 [1,1]=2 → 12
  logic.apply(s, { type: 'roll_dice' }, () => 0);
...
test('购买资金不足 → 募集资金 → 抵押凑够 → 完成购买', () => {
  ...
  p.position = 10;
  logic.apply(s, { type: 'roll_dice' }, () => 0); // 落 12 罗马（无主）
...
test('募资后仍不足则无法确认购买，可取消（城市进入拍卖）', () => {
  ...
  p.cash = 5000;
  p.position = 10;
  logic.apply(s, { type: 'roll_dice' }, () => 0); // 落 12 罗马
  assert.strictEqual(s.phase, 'buy');
...
test('机场购买资金不足 → 募集资金/取消购买', () => {
  ...
  p.cash = 10000;
  p.position = 4;
  logic.apply(s, { type: 'roll_dice' }, () => 0); // [1,1]=2 → 落 6 开罗国际机场
